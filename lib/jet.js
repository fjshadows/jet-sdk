...
JetClient.prototype.call = function(api, query) {
	...
	// @TODO remove it at 21.03.2016
	if(query.SKU && !query.ID) {
		query.ID = query.SKU;
		console.log('[WARNING!] SKU depricated. use ID');
	}

	var ID = query.ID ? query.ID + '/': '';
	if(~api.path.indexOf('merchant-skus')) {
		api.path = api.path + ID + api.action;
	} else {
		api.path = api.path + api.action + ID;
	}
...
exports.Products = require('./orders');
